...
public:
	Plane();
	Plane(Eigen::Vector3d pos, Eigen::Vector3d rotation, Eigen::Vector2d size, double intervall=0.03, bool minimal=false);
	Plane(Eigen::Vector3d cog, Eigen::Vector3d ineriaAxisX, Eigen::Vector3d ineriaAxisY,Eigen::Vector3d ineriaAxisZ, Eigen::Vector3d S, Eigen::Matrix<double,9,1> C, Eigen::Matrix<double,9,1> productMatrix, double bias,std::set<Shared_Point> points);
	~Plane();
	 ...
	 */
	//removed due to QT dependency
	//	boost::shared_ptr<QImage> getHullImages(double &scale);
