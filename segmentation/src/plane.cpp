...
Plane::Plane(Eigen::Vector3d cog, Eigen::Vector3d ineriaAxisX, Eigen::Vector3d ineriaAxisY,Eigen::Vector3d ineriaAxisZ, Eigen::Vector3d S, Eigen::Matrix<double,9,1> C, Eigen::Matrix<double,9,1> productMatrix, double bias,std::set<Shared_Point> points):
	points(points)	
...
Plane::Plane(Eigen::Vector3d pos, Eigen::Vector3d rotation, Eigen::Vector2d size, double intervall, bool minimal){
...
std::list<std::list<std::pair<Shared_Point,Shared_Point> > > Plane::getTransPoints(){
	if(this->transPoints.empty()){
		...
		transPoints = Hull::sortedPair(trans_points);
